...
const MyJobs = () => {
  ...
  const [currentPage, setCurrententPage] = useState(1);
  ...
  async function loadJobs() {
    try {
      setIsLoading(true);
      const jobs = await fetch(`http://localhost:3000/myJobs/${user?.email}`)
        .then((res) => res.json())  
        setJobs(jobs);
        setIsLoading(false);
      ...
      console.log(error)
      alert("Произошла ошибка при загрузке список работы")
    }
  }
  ...
  const nextPage = () => {
    if (indexOfLastItem < jobs.length) {
      setCurrententPage(currentPage + 1)
    }
  }
  ...
  return (
    <div className='max-w-screen-2x1 container mx-auto xl:px-24 px-4'>
      <div className='my-jobs-container'>
        <h1 className='text-center p-4'>Active Jobs</h1>
              ...
              <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded ">
                <div className="rounded-t mb-0 px-4 py-3 border-0">
                  <div className="flex flex-wrap items-center">
                    ...
                    <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
                      <Link to="/post-job">
                        <button className="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button">Post a job</button>
                      </Link>
                    </div>
                  </div>
                </div>
                ...
                <div className="block w-full overflow-x-auto">
                  <table className="items-center bg-transparent w-full border-collapse ">
                    ...
                    {
                      isLoading ? (<tbody className='flex items-center justify-center h-20'><tr><td>Loading....</td></tr></tbody>) :
                        (<tbody>
                          {
                            currentJobs.map((job, index) => (
                              <tr key={index}>
                                ...
                                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left">
                                  <button onClick={() => handleDelete(job._id)} className='bg-red-600 py-2 px-6 text-white rounded-sm'>Delete</button>
                                </td>
